...
export function QuoteTableRowActions<TData>({
  ...
  } = useUpdateUserQuoteStatus(row.getValue("id"));
  const [open, setOpen] = React.useState(false);
  const [value, setValue] = React.useState("");
  const quoteStatus = row.getValue("quoteStatus");
  ...
  if (updateUserQuoteStatusIsPending) {
    return <Skeleton className="w-[170px] h-8" />;
  }
  return (
                ...
                .map((status) => (
                  ...
                  >
                    ...
                    <div className="w-full flex justify-between">
                      {status.label}
                      {status.icon && <status.icon className="ml-2 h-4 w-4" />}
                    </div>
                  </CommandItem>
                ))}
            </CommandGroup>
